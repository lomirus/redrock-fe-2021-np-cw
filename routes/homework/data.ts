...
现给定一接口 \`/homework/fufubot_test/\`，可通过它使用 \`POST\` 方法，form-data 格式向服务器发送任意文本消息（其键值 key 为 "message"），其返回值亦为文本。

- 要求：用户可在前端的输入框输入文本内容，点击发送按钮后发送消息，并将响应信息显示在界面上。

### NEXT LV

fufu 的回复中有机率随机出现下一道题的题目。
`
...
使用此接口用以获取下一等级题目，请求时其请求头中的 \`Authorization\` 需设为 \`BEARER \${token}\`，例：\`BEARER 35rv1AE5rv4fs51f5as1fc5as315as\`.
...
服务器每隔五分钟会向与服务器建立连接的客户端广播一次下一级题目内容。
`;

export const end = 
`卷王，别做了，已经没有题目了😭

不过既然都做完作业了，要不红岩年会了解一下（doge），速来参加🤺
`;